	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Corno(a) saiu do grupo!* @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Se tornou um administrador @${mentioned[0].split('@')[0]}⚡`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
          case 'lilah':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQc4tC_dNqkC-gDJg7pt4uoo42B9extNfBiCA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Punheteiro😳'})
					break
				case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						mentions(`Se tornou um membro comum @${mentioned[0].split('@')[0]}⚡`, mentioned, true)
